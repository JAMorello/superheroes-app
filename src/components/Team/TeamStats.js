...
const TeamStats = ({team}) => {
  ...
  const reduceStat = (heroTeam, stat) => {
    return heroTeam.reduce((sum, sumHero) => sum + parseInt(sumHero.powerstats[stat]), 0)
  }
    ...
    newStats = Object.fromEntries(Object.entries(newStats).sort((a, b) => b[1] - a[1]))
  ...
  useEffect(() => {
    ...
    const number = heroTeam.reduce((sum, sumHero) => sum + parseInt(sumHero.appearance[measure][1].replace(/kgcm/, "")), 0) / heroTeam.length
  ...
  <ul>{Object.entries(stats).map(e => <li>{e[0]}: {e[1]}</li>)}</ul>
  <div>
